/*****************************************************************************
   ...
   gcc -O1 test_2d_kmeans_serial_branchless.c -lm -o test_2d_kmeans_serial_branchless
...
void kmeans(arr_ptr v, arr_ptr weights, arr_ptr centroids, arr_ptr centroids_tmp, data_t *jet_pts, int max_iterations, int convergence_thresh, int *iterations, data_t *total_diff, int k)
{
  ...
  long int dimensions = get_arr_collen(v); // Note: Assumed to be 2
  ...
  for (j = 0; j < row_len; j++)
    assignments[j] = -1;
  int iters = 0;
  data_t min_dist, dist, diff_eta, diff_phi, weight_squared_diff;
  ...
  while ((moved_points > convergence_thresh && iters < max_iterations) || iters == 0)
  {
    ...
    for (j = 0; j < row_len; j++)
    {
      min_dist = 1e30; // Initialize with large value
      min_dist_centroid = 0;
      ...
      for (m = 0; m < k; m++)
      {
        diff_eta = centroid_data[m * 2] - p_eta;
        diff_phi = centroid_data[m * 2 + 1] - p_phi;

        // Branchless Phi Wrap
        diff_phi -= (diff_phi >  M_PI) * (2 * M_PI);
        diff_phi += (diff_phi < -M_PI) * (2 * M_PI);

        dist = (diff_eta * diff_eta) + (diff_phi * diff_phi);

        // Ternary usually compiles to CMOV (Branchless)
      ...
      // Update logic (Phi) with wrap-around correction
      data_t val_phi = p_phi;
      data_t d_phi = val_phi - centroid_data[min_dist_centroid * 2 + 1];
      val_phi -= (d_phi >  M_PI) * (2 * M_PI);
      val_phi += (d_phi < -M_PI) * (2 * M_PI);
    ...
    for (m = 0; m < k; m++)
    {
      if (counts[m] > 0)
      {
        ...
        centroids_tmp_data[m * 2 + 1] /= counts[m];

        // Final Phi wrap-around
        data_t final_phi = centroids_tmp_data[m * 2 + 1];
