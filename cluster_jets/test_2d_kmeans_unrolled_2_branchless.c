/*****************************************************************************
   ...
   gcc -O1 test_2d_kmeans_unrolled_2_branchless.c -lm -o test_2d_kmeans_unrolled_2_branchless
...
void kmeans(arr_ptr v, arr_ptr weights, arr_ptr centroids, arr_ptr centroids_tmp, data_t *jet_pts, int max_iterations, int convergence_thresh, int *iterations, data_t *total_diff, int k)
{
  ...
  long int dimensions = get_arr_collen(v); // Assumed to be 2
  ...
  while ((moved_points > convergence_thresh && iters < max_iterations) || iters == 0)
  {
    ...
    for (j = 0; j < row_len; j++)
    {
      ...
      // 2-accumulator unroll
      for (m = 0; m < k - 1; m += 2)
      {
        // Centroid A
        data_t da_eta = centroid_data[m * 2] - p_eta;
        data_t da_phi = centroid_data[m * 2 + 1] - p_phi;
        da_phi -= (da_phi >  M_PI) * (2 * M_PI);
        da_phi += (da_phi < -M_PI) * (2 * M_PI);
        data_t dist_a = (da_eta * da_eta) + (da_phi * da_phi);

        // Centroid B
        data_t db_eta = centroid_data[(m + 1) * 2] - p_eta;
        data_t db_phi = centroid_data[(m + 1) * 2 + 1] - p_phi;
        db_phi -= (db_phi >  M_PI) * (2 * M_PI);
        db_phi += (db_phi < -M_PI) * (2 * M_PI);
        data_t dist_b = (db_eta * db_eta) + (db_phi * db_phi);

        // Branchless min-update for A
        int is_closer_a = (dist_a < min_dist);
        min_dist = is_closer_a ? dist_a : min_dist;
        min_dist_centroid = is_closer_a ? m : min_dist_centroid;

        // Branchless min-update for B
        int is_closer_b = (dist_b < min_dist);
        min_dist = is_closer_b ? dist_b : min_dist;
        min_dist_centroid = is_closer_b ? (m + 1) : min_dist_centroid;
      }

      // Cleanup leftover for odd k
      for (; m < k; m++)
      {
        data_t d_eta = centroid_data[m * 2] - p_eta;
        data_t d_phi = centroid_data[m * 2 + 1] - p_phi;
        d_phi -= (d_phi >  M_PI) * (2 * M_PI);
        d_phi += (d_phi < -M_PI) * (2 * M_PI);
        data_t dist = (d_eta * d_eta) + (d_phi * d_phi);

        int is_closer = (dist < min_dist);
      ...
      // Centroid Update Accumulation (Branchless Wrap)
