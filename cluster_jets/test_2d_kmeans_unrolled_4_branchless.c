/*****************************************************************************
   ...
   gcc -O1 test_2d_kmeans_unrolled_4_branchless.c -lm -o test_2d_kmeans_unrolled_4_branchless
...
/* K_MAX-means with 4-accumulator unrolling */
void kmeans(arr_ptr v, arr_ptr weights, arr_ptr centroids, arr_ptr centroids_tmp, data_t *jet_pts, int max_iterations, int convergence_thresh, int *iterations, data_t *total_diff, int k)
{
  ...
  while ((moved_points > convergence_thresh && iters < max_iterations) || iters == 0)
  {
    ...
    for (j = 0; j < row_len; j++)
    {
      ...
      // 4-accumulator unroll
      for (m = 0; m <= k - 4; m += 4)
      {
        // Math Block: Independent registers for 4 centroids
        data_t de0 = centroid_data[m * 2] - p_eta;
        data_t dp0 = centroid_data[m * 2 + 1] - p_phi;
        dp0 -= (dp0 > M_PI) * (2 * M_PI); dp0 += (dp0 < -M_PI) * (2 * M_PI);
        data_t dist0 = (de0 * de0) + (dp0 * dp0);

        data_t de1 = centroid_data[(m+1) * 2] - p_eta;
        data_t dp1 = centroid_data[(m+1) * 2 + 1] - p_phi;
        dp1 -= (dp1 > M_PI) * (2 * M_PI); dp1 += (dp1 < -M_PI) * (2 * M_PI);
        data_t dist1 = (de1 * de1) + (dp1 * dp1);

        data_t de2 = centroid_data[(m+2) * 2] - p_eta;
        data_t dp2 = centroid_data[(m+2) * 2 + 1] - p_phi;
        dp2 -= (dp2 > M_PI) * (2 * M_PI); dp2 += (dp2 < -M_PI) * (2 * M_PI);
        data_t dist2 = (de2 * de2) + (dp2 * dp2);

        data_t de3 = centroid_data[(m+3) * 2] - p_eta;
        data_t dp3 = centroid_data[(m+3) * 2 + 1] - p_phi;
        dp3 -= (dp3 > M_PI) * (2 * M_PI); dp3 += (dp3 < -M_PI) * (2 * M_PI);
        data_t dist3 = (de3 * de3) + (dp3 * dp3);

        // Comparison Block: Serial CMOV chain
        int c0 = (dist0 < min_dist);
        min_dist = c0 ? dist0 : min_dist;
        min_dist_centroid = c0 ? m : min_dist_centroid;

        int c1 = (dist1 < min_dist);
        min_dist = c1 ? dist1 : min_dist;
        min_dist_centroid = c1 ? (m+1) : min_dist_centroid;

        int c2 = (dist2 < min_dist);
        min_dist = c2 ? dist2 : min_dist;
        min_dist_centroid = c2 ? (m+2) : min_dist_centroid;

        int c3 = (dist3 < min_dist);
        min_dist = c3 ? dist3 : min_dist;
        min_dist_centroid = c3 ? (m+3) : min_dist_centroid;
      }

      // Cleanup leftover for k
      for (; m < k; m++)
      {
        data_t de = centroid_data[m * 2] - p_eta;
        data_t dp = centroid_data[m * 2 + 1] - p_phi;
        dp -= (dp > M_PI) * (2 * M_PI); dp += (dp < -M_PI) * (2 * M_PI);
        data_t d = (de * de) + (dp * dp);
        int c = (d < min_dist);
        min_dist = c ? d : min_dist;
        min_dist_centroid = c ? m : min_dist_centroid;
      }
