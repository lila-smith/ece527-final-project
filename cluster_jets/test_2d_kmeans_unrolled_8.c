/*****************************************************************************
   ...
   gcc -O1 test_2d_kmeans_unrolled_8.c -lm -o test_2d_kmeans_unrolled_8
  ...
  while ((moved_points > convergence_thresh && iters < max_iterations) || iters == 0)
  {
    ...
    for (j = 0; j < row_len; j++)
    {
      ...
      /* Find the closest centroid using squared Euclidean distance. 
         8-accumulator unroll: process four centroids per iteration. */      

      /******* START DIFF W/ SERIAL *******/

      min_dist = -1.0;
      min_dist_centroid = 0;
      /* 8-accumulator unroll: process eight centroids per iteration */
      for (m = 0; m < k - 7; m += 8)
      {
        data_t dist_a = 0.0, dist_b = 0.0, dist_c = 0.0, dist_d = 0.0;
        data_t dist_e = 0.0, dist_f = 0.0, dist_g = 0.0, dist_h = 0.0;
        for (i = 0; i < dimensions; i++)
        {
          data_t pt_i = data[j * dimensions + i];
          data_t diff_a = centroid_data[m * dimensions + i]       - pt_i;
          data_t diff_b = centroid_data[(m + 1) * dimensions + i] - pt_i;
          data_t diff_c = centroid_data[(m + 2) * dimensions + i] - pt_i;
          data_t diff_d = centroid_data[(m + 3) * dimensions + i] - pt_i;
          data_t diff_e = centroid_data[(m + 4) * dimensions + i] - pt_i;
          data_t diff_f = centroid_data[(m + 5) * dimensions + i] - pt_i;
          data_t diff_g = centroid_data[(m + 6) * dimensions + i] - pt_i;
          data_t diff_h = centroid_data[(m + 7) * dimensions + i] - pt_i;
          if (i == 1)
          {
            ...
            if (diff_d < -M_PI) diff_d += 2 * M_PI;
            if (diff_e > M_PI)  diff_e -= 2 * M_PI;
            if (diff_e < -M_PI) diff_e += 2 * M_PI;
            if (diff_f > M_PI)  diff_f -= 2 * M_PI;
            if (diff_f < -M_PI) diff_f += 2 * M_PI;
            if (diff_g > M_PI)  diff_g -= 2 * M_PI;
            if (diff_g < -M_PI) diff_g += 2 * M_PI;
            if (diff_h > M_PI)  diff_h -= 2 * M_PI;
            if (diff_h < -M_PI) diff_h += 2 * M_PI;
          }
          ...
          dist_e += diff_e * diff_e;
          dist_f += diff_f * diff_f;
          dist_g += diff_g * diff_g;
          dist_h += diff_h * diff_h;
        }
        ...
        if (dist_e < min_dist)                   { min_dist = dist_e; min_dist_centroid = m + 4; }
        if (dist_f < min_dist)                   { min_dist = dist_f; min_dist_centroid = m + 5; }
        if (dist_g < min_dist)                   { min_dist = dist_g; min_dist_centroid = m + 6; }
        if (dist_h < min_dist)                   { min_dist = dist_h; min_dist_centroid = m + 7; }
      }
      /* Handle leftover centroids (0-7 of them) */
      for (; m < k; m++)
      {
        dist = 0.0;
        for (i = 0; i < dimensions; i++)
